...
import { fetchCyInventReport } from "../services/cyInventService";


export default function CylinderReport() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try { 
        const res = await fetchCyInventReport();
        setData(res);
      } catch (error) {
        console.error("Error fetching cylinder report", error);
      }
    };
...
const exportToPDF = async () => {
  ...
  try {
    ...
    doc.setFontSize(16);
    doc.text("Reporte de Inventario de Helados", pageWidth / 2, 20, { align: "center" });
    ...
    const requests = Array.from({ length: totalPages }, (_, page) =>
          fetchCyInventReport(page, itemsPerPage)
        );
    ...
    // === Guardar PDF ===
    doc.save(`reporte_inventario_cylinders.pdf`);
  ...
  // Guarda el archivo
  XLSX.writeFile(wb, "reporte_inventario_cylinders.xlsx");
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-center mb-6 text-primary">📊 Reporte de Inventario de Helados</h1>

      {/* Filtro de fechas */}
      <div className="card shadow-lg p-4 mb-6 rounded-xl bg-base-100">
        <h2 className="text-xl font-semibold text-center mb-4">Obtener Inventario de Helados</h2>
        ...
        <table className="table table-zebra w-full">
          <thead>
            <tr className="bg-primary text-white">
              <th className="text-center">Helado</th>
          ...
          <tbody>
            {getCurrentData().map((r, idx) => (
              <tr key={idx}>
                <td className="text-center">{r.flavor}</td>
                <td className="text-center">{r.status}</td>
                <td className="text-center">{r.fraction * 100} %</td>
